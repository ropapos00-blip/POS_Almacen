-- 38_wholesale_qoh_fallback_fix.sql
-- Extiende el fallback a quantity_on_hand para facturas de confeccion sin talla/color.
-- Cuando color='UNICO' no existe en color_quantities, usa quantity_on_hand como disponible.
-- Corrige tambien el restore en update para no crear entradas fantasma en size/color matrices.
...
begin
  ...
  loop
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      if v_has_color_matrix then
        if coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color then
          -- Color found in matrix: use specific slot
          v_available_qty := coalesce((coalesce(v_ref.color_quantities, '{}'::jsonb) -> v_color ->> v_size)::integer, 0);
        else
          -- Color not in matrix (e.g. UI simplified, no color selected): fallback to quantity_on_hand
  ...
  loop
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      -- Determine deduction strategy
      v_use_qoh_fallback := false;
      if v_has_color_matrix then
        if not (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color) then
          -- Color not in matrix: only deduct quantity_on_hand
          v_use_qoh_fallback := true;
        end if;
      else
        v_use_qoh_fallback := (
      ...
      if v_has_color_matrix and not v_use_qoh_fallback then
        v_available_qty := coalesce((coalesce(v_ref.color_quantities, '{}'::jsonb) -> v_color ->> v_size)::integer, 0);
        ...
        -- Simple reference without size/color tracking: deduct only from quantity_on_hand
        update public.wholesale_references
        set
          quantity_on_hand = greatest(0, coalesce(quantity_on_hand, 0) - v_qty),
          updated_at = timezone('America/Bogota', now())
...
begin
  ...
  loop
    ...
    if found then
      ...
      if v_has_color_matrix and (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color) then
        -- Color exists in matrix: restore all three tracking fields
        update public.wholesale_references
        set
          ...
          updated_at = timezone('America/Bogota', now())
        where id = v_ref.id;
      elsif not v_has_color_matrix and (coalesce(v_ref.size_quantities, '{}'::jsonb) ? v_size) then
        -- Size exists in size_quantities: restore size + quantity_on_hand
        update public.wholesale_references
        set
          quantity_on_hand = coalesce(quantity_on_hand, 0) + v_old_item.quantity,
          size_quantities = jsonb_set(
        ...
        -- No matching size/color slot (qoh-fallback item): restore only quantity_on_hand
        update public.wholesale_references
        set
          quantity_on_hand = coalesce(quantity_on_hand, 0) + v_old_item.quantity,
          updated_at = timezone('America/Bogota', now())
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      if v_has_color_matrix then
        if coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color then
          ...
          -- Color not in matrix: fallback to quantity_on_hand
          v_available_qty := coalesce(v_ref.quantity_on_hand, 0);
          v_use_qoh_fallback := true;
        end if;
      else
