-- 42_fix_color_not_in_matrix_fallback.sql
--
-- Problema: cuando una referencia tiene color_quantities no vacío (v_has_color_matrix=true)
-- pero la clave de color recibida (ej. 'UNICO') NO existe en el JSON, el RPC leía
-- color_quantities -> 'UNICO' ->> 'UNICA' = NULL → 0 y lanzaba "Stock insuficiente"
-- aunque quantity_on_hand fuera 287.
--
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      if v_has_color_matrix then
        if (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color) then
          -- Color key found in matrix: use specific color/size stock
          v_available_qty := coalesce((coalesce(v_ref.color_quantities, '{}'::jsonb) -> v_color ->> v_size)::integer, 0);
        else
          -- Color key NOT in matrix (e.g. 'UNICO' sent but reference has real colors):
          -- fall back to total quantity_on_hand
  ...
  loop
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      insert into public.wholesale_invoice_items (
        ...
        v_description, null, null, v_description, v_qty, v_unit_price, v_line_total
      );
    else
      ...
      -- QOH fallback: (a) has color matrix but color key not in it, OR
      --               (b) no color matrix and size not in size_quantities
      v_use_qoh_fallback := (
        v_has_color_matrix
        and not (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color)
      ) or (
        not v_has_color_matrix and (
          coalesce(v_ref.size_quantities, '{}'::jsonb) = '{}'::jsonb
          or not (coalesce(v_ref.size_quantities, '{}'::jsonb) ? v_size)
        )
      );
      ...
      if v_use_qoh_fallback then
        -- Only deduct from quantity_on_hand; do not touch color/size matrices
...
begin
  ...
  loop
    ...
    if found then
      ...
      -- QOH fallback restore: color key not in matrix, or no matrix and size not in size_quantities
      v_use_qoh_fallback := (
        v_has_color_matrix
        and not (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_old_item.color)
      ) or (
        not v_has_color_matrix and (
          coalesce(v_ref.size_quantities, '{}'::jsonb) = '{}'::jsonb
    ...
    if coalesce(v_item ->> 'reference_id', '') = '' then
      ...
      if v_has_color_matrix then
        if (coalesce(v_ref.color_quantities, '{}'::jsonb) ? v_color) then
          ...
          -- Color key NOT in matrix: fall back to total quantity_on_hand
          v_available_qty := coalesce(v_ref.quantity_on_hand, 0);
          v_use_qoh_fallback := true;
        end if;
      else
